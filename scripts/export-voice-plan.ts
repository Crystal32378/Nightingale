...
const guide = [
  ...
  ...VOICE_GENDERS.map(
    (g) =>
      `- **${g}** — 音色 \`${VOICE_PROFILES[g].voiceId}\`，speed ${VOICE_PROFILES[g].speed}、pitch ${VOICE_PROFILES[g].pitch}、volume ${VOICE_PROFILES[g].vol} → 存到 \`public/${audioDirFor(g)}/\``,
  ),
  '',
  ...
  '| # | 要生成的文字 | 存成這個檔名 |',
  '| --- | --- | --- |',
  ...canonical.map((u, i) => `| ${i + 1} | \`${u.text}\` | \`${u.fileStem}.mp3\` |`),
  '',
